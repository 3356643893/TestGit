...
SUITE = "api"
TEST_DIR_NAME = "test_api"


def run():
    """API 测试套件入口：pytest 执行 test_api 目录"""
    ...
    if pytest_exit == 0:
        logger.info("API pytest 执行完毕：全部通过")
    else:
        logger.warning(f"API pytest 执行完毕：存在失败用例 (返回码={pytest_exit})")
    ...
    logger.info(f"API 测试报告已生成: {report_index}")
    ...
    if pytest_exit != 0:
        ...
        try:
            ...
            send_alert_email(
                subject=f"[告警] API 自动化测试 {failed_count}/{total_count} 用例失败",
                html_body=html_body
            )
