...
SUITE = "performance"
TEST_DIR_NAME = "test_performance"


def run():
    """性能测试套件入口：pytest 执行 test_performance 目录"""
    ...
    if pytest_exit == 0:
        logger.info("性能 pytest 执行完毕：全部通过")
    else:
        logger.warning(f"性能 pytest 执行完毕：存在失败用例 (返回码={pytest_exit})")
    ...
    logger.info(f"性能测试报告已生成: {report_index}")
    ...
    if pytest_exit != 0:
        ...
        try:
            ...
            send_alert_email(
                subject=f"[告警] 性能测试 {failed_count}/{total_count} 用例失败",
                html_body=html_body
            )
